...
function BookFlight({ bookFlightData }) {
    ...
    const { isAuthenticated, userName, email,backendUrl } = useContext(AuthContext);
    ...
    const bookFlight = async () => {
        ...
        try {
            const res = await fetch(backendUrl+'/api/payment/getkey', {
                method: 'GET',
                headers: {
            ...
            const options = {
                ...
                // callback_url: "http://localhost:8080/api/payment/paymentverification",
                 handler: async function (response) {
                    ...
                    const res1 = await fetch(backendUrl+"/api/payment/paymentverification", {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    const checkoutData=()=>{
        const discount = Math.floor(Math.random() * (12 - 5 + 1)) + 5;
        navigate('/checkout', {
            state: { 
