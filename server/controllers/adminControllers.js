...
async function addFlight(req, res) {
    const { flightNo, to, from, category, totalSeats,totalPrice, date, departureTime, arrivalTime, airline } = req.body;
    const date_ = new Date(date);
    try {
        if (req.user.role !== 'admin') {
            return res.status(400).json({ message: "Not Authorized" });
        }
        const flight = new Flight({ flightNo, to, from, category, totalSeats,totalPrice, date: date_, departureTime, arrivalTime, airline });
        await flight.save();
...
async function editFlight(req, res) {
    ...
    try {
        ...
        if (totalPrice) flight.totalPrice = totalPrice;
